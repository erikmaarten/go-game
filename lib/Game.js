...
Game.capture = function(board, playedPosition) {
  ...
  var otherPlayer = playerType === "1" ? "2" : "1";

  var newBoard = _captureColor(board, adjacent, playerType, boardWidth);
  // Check for self-capture (suicide)
  // included the position played
  adjacent.push(stringPos);
  newBoard = _captureColor(board, adjacent, playerType, boardWidth);
